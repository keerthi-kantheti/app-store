...
class AppStore extends Component {
  ...
  getFilteredApps = () => {
    const {selectedTab, initialList, searchInput} = this.state
    const reqAppsList = initialList.filter(
      eachItem => selectedTab === eachItem.category,
    )
    const reqAppsListAfterSearch = reqAppsList.filter(each =>
      each.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )

    return reqAppsListAfterSearch
  }

  render() {
    const {searchInput, initialList, selectedTab} = this.state
